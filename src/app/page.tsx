...
import { Plus, MagnifyingGlass, Check, X, MathOperations, Palette, Scissors, Wrench, Ruler, HardHat, PencilSimple } from "@phosphor-icons/react";
...
export default function Orcamentos() {
  ...
  const [dbTaxas, setDbTaxas] = useState<any>({ inst_padrao: 45, inst_alta: 56 }); // Fallback caso o banco demore
  ...
  const [ferragemId, setFerragemId] = useState("nenhum"); // Agora é dinâmico pelo ID
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    if (!tecObj || !forObj || !modObj || !ferObj) return alert("Selecione os materiais e o modelo.");
    if (tecObj.id === 'nenhum' && forObj.id === 'nenhum') return alert("Selecione tecido ou forro.");

    let detalhes = [];
    
    // 1. Cálculo Tecido (METRO LINEAR)
    let custoTec = 0;
    if (tecObj.id !== 'nenhum') {
    ...
    // 4. Cálculo Ferragem Dinâmica
    let custoFer = 0;
    if (ferObj.id !== 'nenhum') {
      custoFer = largNum * ferObj.preco;
      detalhes.push({ tipo: 'Ferragem', icon: <Wrench size={20} className="text-gray-500" />, nome: ferObj.nome, equacao: `${largNum}m × ${formatBRL(ferObj.preco)}/m`, valor: custoFer });
    }
  ...
  // --- TOTAIS GLOBAIS (USANDO TAXAS DO BANCO E REGRA > 3.5m) ---
  const totais = useMemo(() => {
    ...
    cart.forEach(item => {
      ...
      if (item.servico === 'padrao') { 
        if (item.altura > 3.50) mtAlta += item.largura; 
        else mtPadrao += item.largura; 
      }
      if (item.servico === 'so_pendurar') mtPendurar += item.largura;
      if (item.servico === 'retirar') { mtPadrao += item.largura; mtRetirada += item.largura; }
    });

    // Pega as taxas do banco, garantindo o fallback pedido de 45 e 56
    const precoPadrao = Number(dbTaxas.inst_padrao) || 45;
    const precoAlto = Number(dbTaxas.inst_alta) || 56;
    const precoRetirada = Number(dbTaxas.retirada) || 20;

    let calcInst = (mtPadrao * precoPadrao) + (mtAlta * precoAlto) + (mtRetirada * precoRetirada);
    
    if (mtPadrao > 0) globalDetalhes.push({ nome: 'Instalação Padrão (Até 3.5m)', desc: `${mtPadrao.toFixed(2)}m`, valor: mtPadrao * precoPadrao });
    if (mtAlta > 0) globalDetalhes.push({ nome: 'Pé Direito Alto (> 3.5m)', desc: `${mtAlta.toFixed(2)}m`, valor: mtAlta * precoAlto });
    if (mtRetirada > 0) globalDetalhes.push({ nome: 'Taxa de Retirada', desc: `${mtRetirada.toFixed(2)}m`, valor: mtRetirada * precoRetirada });
  ...
  // --- FINALIZAR OU EDITAR PEDIDO E REDIRECIONAR ---
  const finalizarPedido = async () => {
    ...
    if (!error) { 
      ...
      setKm(0); 
      setEditId(null); 
      // Já redireciona para o histórico logo após salvar
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-6">
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <input type="text" placeholder="Nome do Cliente" value={cliente} onChange={(e) => setCliente(e.target.value)} className="flex-1 p-3 border border-gray-300 rounded-md outline-none focus:border-blue-500" />
                <input type="number" placeholder="KM (Ida)" value={km} onChange={(e) => setKm(Number(e.target.value))} className="w-full md:w-32 p-3 border border-gray-300 rounded-md outline-none focus:border-blue-500" />
              </div>
            </div>

            <div className={`bg-white p-4 md:p-6 rounded-lg border shadow-sm transition-all duration-300 ${editingCartItemId ? 'border-indigo-400 ring-4 ring-indigo-50' : 'border-gray-200'}`}>
              ...
              <form onSubmit={handleAddItem} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input type="text" value={nomeAmbiente} onChange={(e)=>setNomeAmbiente(e.target.value)} placeholder="Ambiente (Ex: Sala)" className="p-3 border border-gray-300 rounded-md outline-none focus:border-indigo-500" required />
                  <input type="number" step="0.01" value={largura} onChange={(e)=>setLargura(e.target.value)} placeholder="Largura (m)" className="p-3 border border-gray-300 rounded-md outline-none focus:border-indigo-500" required />
                  <input type="number" step="0.01" value={altura} onChange={(e)=>setAltura(e.target.value)} placeholder="Altura (m)" className="p-3 border border-gray-300 rounded-md outline-none focus:border-indigo-500" required />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <select value={tecidoId} onChange={(e)=>setTecidoId(e.target.value)} className="p-3 border border-gray-300 rounded-md bg-white outline-none focus:border-indigo-500">
                    {dbTecidos.map(t => <option key={t.id} value={t.id}>{t.nome} (R$ {t.preco}/m)</option>)}
                  </select>
                  <select value={forroId} onChange={(e)=>setForroId(e.target.value)} className="p-3 border border-gray-300 rounded-md bg-white outline-none focus:border-indigo-500">
                    {dbForros.map(f => <option key={f.id} value={f.id}>{f.nome} (R$ {f.preco}/m)</option>)}
                  </select>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <select value={modeloId} onChange={(e)=>setModeloId(e.target.value)} className="p-3 border border-gray-300 rounded-md bg-white outline-none focus:border-indigo-500">
                    {dbModelos.map(m => <option key={m.id} value={m.id}>{m.nome} (Fator {m.fator})</option>)}
                  </select>
                  <select value={ferragemId} onChange={(e)=>setFerragemId(e.target.value)} className="p-3 border border-gray-300 rounded-md bg-white outline-none focus:border-indigo-500">
                    {dbFerragens.map(f => <option key={f.id} value={f.id}>{f.nome}</option>)}
                  </select>
                </div>

                <button type="submit" className={`w-full p-4 text-white rounded-md font-bold flex items-center justify-center gap-2 transition-colors shadow-sm ${editingCartItemId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm md:sticky md:top-6 h-fit">
            ...
            {cart.length === 0 ? (
              ...
              cart.map(item => (
                <div key={item.id} className={`border p-3 rounded-md relative group transition-colors ${editingCartItemId === item.id ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 bg-gray-50 hover:bg-white'}`}>
                  ...
                  <div className="absolute top-2 right-2 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition">
                    <button type="button" onClick={() => editCartItem(item)} className="p-1.5 bg-white text-indigo-500 border border-indigo-200 rounded hover:bg-indigo-500 hover:text-white transition shadow-sm">
                      <PencilSimple size={16} weight="bold" />
                    </button>
                    <button type="button" onClick={() => delItem(item.id)} className="p-1.5 bg-white text-red-500 border border-red-200 rounded hover:bg-red-500 hover:text-white transition shadow-sm">
          ...
          <button onClick={() => { if (cart.length === 0) return alert("Adicione itens primeiro!"); setIsModalOpen(true); }} className="w-full mt-6 p-3 bg-transparent border border-gray-300 text-gray-700 rounded-md font-semibold flex items-center justify-center gap-2 hover:bg-gray-50 transition">
            <MagnifyingGlass size={20} /> Ver Detalhes
          </button>
          
          <button onClick={finalizarPedido} className={`w-full mt-3 p-4 text-white rounded-md font-bold shadow-md transition flex items-center justify-center gap-2 ${editId ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-500 hover:bg-emerald-600'}`}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl relative">
            <div className="sticky top-0 bg-white px-4 md:px-8 py-4 md:py-6 border-b flex justify-between items-center z-10">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-gray-800 flex items-center gap-2">
                  <MathOperations size={24} className="text-blue-600" /> Memória de Cálculo
                </h2>
              </div>
            ...
            <div className="p-4 md:p-8 space-y-6 md:space-y-8">
              {cart.map((item, idx) => (
                <div key={item.id} className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="bg-gray-50 px-4 py-3 border-b flex flex-col md:flex-row md:justify-between md:items-center gap-1">
                  ...
                  <div className="divide-y divide-gray-100">
                    {item.detalhes_array?.map((det: any, i: number) => (
                      <div key={i} className="px-4 py-4 flex flex-col md:flex-row md:items-start gap-3 md:gap-4 hover:bg-gray-50 transition">
                        <div className="p-2 bg-white border border-gray-100 rounded-md shadow-sm w-fit">{det.icon}</div>
                        ...
                        <div className="text-sm font-semibold text-gray-800 md:text-right">{formatBRL(det.valor)}</div>
                      </div>
                    ))}
                  </div>

                  <div className="bg-blue-50/50 px-4 py-3 border-t flex justify-end items-center gap-4">
              ...
              <div className="border border-emerald-200 rounded-lg overflow-hidden">
                ...
                <div className="divide-y divide-emerald-100 bg-white">
                  {totais.globalDetalhes.length === 0 ? (
                    <div className="px-4 py-4 text-sm text-gray-500 italic">Nenhum serviço extra acionado.</div>
                  ) : (
                    totais.globalDetalhes.map((serv: any, i: number) => (
                      <div key={i} className="px-4 py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                        <div>
                ...
                <div className="bg-emerald-50/50 px-4 py-3 border-t border-emerald-200 flex justify-end items-center gap-4">
